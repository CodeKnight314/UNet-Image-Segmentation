...
class DiceLoss(nn.Module):
    ...
    def forward(self, prediction, truth):
        ...
        for i in range(self.num_classes):
            ...
            dice_coefficient = (2. * intersection + self.smoothing) / (
...
class FocalLoss(nn.Module): 
    def __init__(self, alpha = None, gamma = 2, reduction : str = "mean"): 
        ...
        if self.alpha is not None:
            if isinstance(self.alpha, (list, tuple, torch.Tensor)):
                if not isinstance(self.alpha, torch.Tensor):
                    self.alpha = torch.tensor(self.alpha, dtype=prediction.dtype, device=prediction.device)
